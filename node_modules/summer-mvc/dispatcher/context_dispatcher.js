...
exports.dispatching = function( req, res ){
  ...
  return new Promise( function(resolve, reject){
    ...
    // let dispatchingSpec = getDispatchingSpec( "", req._parsedUrl.pathname );

    let view = require("path").join( dispatchingSpec.viewPath, dispatchingSpec.view );

    getControlFunction( dispatchingSpec );
...
function getControlFunction( dispatchingSpec ){
  let fs = require( "fs" );

  let controllerPath = require("path").join( process.cwd(), "controller", "controller_basic.js" );
  let controllerBody = fs.readFileSync( controllerPath, "utf-8" );

  let controllerBodySplited = controllerBody.split( "\n" );

  for( var i=0; i<controllerBodySplited.length; i++ ){

    if( controllerBodySplited[i].indexOf( "/*" ) == 0 ){
      console.log( i + " : " + controllerBodySplited[i] );
    }
    if( controllerBodySplited[i].indexOf( "//" ) == 0 ){
      console.log( i + " : " + controllerBodySplited[i] );
    }
  ...
  console.log( controllerBodySplited );
}
